...
double kp = 1, kd = 0.7, wo = 0.05;

double ADRCV(double error, double last_error, double u, double y)
{
    double derivative_e = error - last_error;
    double u_o = 1 * error + 1 * derivative_e;
    ...
    x1v = x1v + dx1;
    // Log estimated states from ADRC
    x1v_log.push_back(x1v);

    double dx2 = -(L2 * e1);
    x2v = x2v + dx2;
    return ((u_o - x2v) * 0.0833);
}
double ADRCW(double error, double last_error, double u, double y)
{
    double derivative_e = error - last_error;
    double u_o = 1.9 * error + 1 * derivative_e;
    ...
    x1w = x1w + dx1;
    x1w_log.push_back(x1w);

    double dx2 = -(L2 * e1);
    x2w = x2w + dx2;
    return ((u_o - x2w) * 0.0833);
}
...
void followCircularTrajectory()
{
    ...
    while (ros::ok())
    {
        ...
        v_dd = (0.1 * e_x) + (v_r * cos(e_theta));
        w_dd = (0.8 * v_r * e_y) + w_r + (1.3 * sin(e_theta));
...
int main(int argc, char **argv)
{
    ...
    pub = nh.advertise<geometry_msgs::Twist>("/pioneer/cmd_vel", 1000);
    ...
    FILE *gnuplotPipe5 = popen("gnuplot -persistent", "w");
    if (gnuplotPipe5)
    {
        fprintf(gnuplotPipe5, "set title 'ADRC Estimated States'\n");
        fprintf(gnuplotPipe5, "set xlabel 'Time Step'\n");
        fprintf(gnuplotPipe5, "set ylabel 'Estimated States'\n");
        fprintf(gnuplotPipe5, "plot '-' with lines title 'x1v (Velocity Estimate)' lc rgb 'blue', '-' with lines title 'x1w (Angular Velocity Estimate)' lc rgb 'red'\n");

        // Plot x1v
        for (size_t i = 0; i < x1v_log.size(); i++)
        {
            fprintf(gnuplotPipe5, "%lu %f\n", i, x1v_log[i]);
        }
        fprintf(gnuplotPipe5, "e\n");

        // Plot x1w
        for (size_t i = 0; i < x1w_log.size(); i++)
        {
            fprintf(gnuplotPipe5, "%lu %f\n", i, x1w_log[i]);
        }
        fprintf(gnuplotPipe5, "e\n");
    ...
    {
        ROS_ERROR("Could not open Gnuplot for ADRC estimated states.");
    }
