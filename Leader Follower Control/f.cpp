...
geometry_msgs::Pose follower_pose;
double follower_v = 0.0;
double follower_w = 0.0;
...
void followerOdomCallback(const nav_msgs::Odometry::ConstPtr& msg) {
  follower_pose = msg->pose.pose;
  odom_received = true;
  follower_v = msg->twist.twist.linear.x;
  follower_w = msg->twist.twist.angular.z;
}
...
void followCircularTrajectory() {
  ...
  while (ros::ok()) {
      ...
      // Relative position between leader and follower
      x_ref = leader_pose.position.x;
      y_ref = leader_pose.position.y;
      ...
      //follower position
      x_m = follower_pose.position.x;
      y_m = follower_pose.position.y;
      ...
      // Direction to leader
      double theta_r = atan2(ey, ex);
      theta_rr.push_back(theta_r);

      // Follower's current orientation
      double theta_m = tf::getYaw(follower_pose.orientation);
      theta_mm.push_back(theta_m);

      // Normalize angle
      double e_theta = theta_r - theta_m;
      ...
      // // Clamp speeds
      // if (cmd_vel.linear.x > 0.5) cmd_vel.linear.x = 0.5;
      ...
      // w_d = (0.9 * v_r * e_y) + w_r + (0.4 * sin(e_theta));
      ...
      // Log velocities
      lref.push_back(v_d);
      lreal.push_back(follower_v);
      aref.push_back(w_d);
      areal.push_back(follower_w);
...
int main(int argc, char **argv) {
  ...
  ros::Subscriber leader_sub = nh.subscribe("/leader/odom", 10, leaderOdomCallback);
  ros::Subscriber follower_sub = nh.subscribe("/follower/odom", 10, followerOdomCallback);
  follower_pub = nh.advertise<geometry_msgs::Twist>("/follower/cmd_vel", 10);
  ...
  ROS_INFO("Odometry data received. Starting to follow leader.");
